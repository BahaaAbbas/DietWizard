...
class WeeklyTemdailySummary extends StatefulWidget {
 ...
 final String TraineeEmail;
  ...
  @override
  State<WeeklyTemdailySummary> createState() => _WeeklyTemdailySummaryState();
}

class _WeeklyTemdailySummaryState extends State<WeeklyTemdailySummary> {
 ...
 //CalTotCaloriesdayofWeek()
 Future<void> CalTotCaloriesdayofWeek() async {
    try {
      var url = Uri.parse('${widget.baseUrl}/CalTotCaloriesdayofWeek');
...
percentageCaloriesLunch = 0.00;
 ...
 //CalTotGoalLeftdayofWeek()
 Future<void> CalTotGoalLeftdayofWeek() async {
    try {
      var url = Uri.parse('${widget.baseUrl}/CalTotGoalLeftdayofWeek');
                 var response = await http.post(
            url,
            body: jsonEncode({
              'coachemail': widget.CoachEmail,
  ...
  void initState() {
    ...
    selectedDate = widget.selectedDateDisplay;
       // DateTime.now(); // Set the default selected date to current date
            ...
            _isAllSelected == true ? CalTotCaloriesdayofWeek() : CalTotGoalLeftdayofWeek() ;
  }
        ...
        child: Column(
          ...
          children: [
            ...
            _isAllSelected?
                  ...
                  CaloriesDailySummary(name: widget.name,baseUrl: widget.baseUrl,
                                       TraineeEmail: widget.TraineeEmail,
                      CoachEmail: widget.CoachEmail,)
                  : 
...
//GlobalMealNameForMacros
 ...
 //CalTotGivenNutdayofWeek()
 Future<void> CalTotGivenNutdayofWeek() async {
    try {
      var url = Uri.parse('${widget.baseUrl}/CalTotGivenNutdayofWeek');
                var response = await http.post(
            url,
            body: jsonEncode({
              'coachemail': widget.CoachEmail,
